...
//4. 상품 디테일을 눌렀으나, 로그인이 안 돼 있을 경우에는 로그인 페이지가 먼저 나온다.
//5. 로그인이 되어 있을 경우에는 상품 디테일 페이지를 볼 수 있다.
//6. 로그아웃 버튼을 클릭하면 로그아웃이 됨
//7. 로그아웃이 되면 상품 디테일 페이지를 볼 수 없다, 다시 로그인 페이지가 보인다.
//8. 로그인을 하면 로그아웃이 보이고, 로그아웃을 하면 로그인이 보인다.
//9. 상품을 검색할 수 있다.
function App() {
  const [authenticate, setAuthenticate] = useState(false); //true면 로그인 됨 false면 안 됨
  useEffect(() => {
    console.log('Aaaa', authenticate);
  }, [authenticate]);
  return (
    <div>
      {/* Navbar 아래에 있는 내용만 routes 통해 바뀔 예정 */}
      <Navbar />
      <Routes>
